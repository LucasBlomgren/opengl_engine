...
void PhysicsEngine::createPredictedIslandsMVP(float frameDt)
{
    ...
    // Reset marks only for awake bodies.
    ...
    for (RigidBodyHandle h : awake) {
        ...
        // Static or terrain hit creates at least a singleton island.
        bool hitTerrain =
            broadphaseManager.getTerrainBVH().queryAny(
                risk.sweptAABB
            );

        bool hitStatic =
            broadphaseManager.getStaticBVH().queryAny(
                risk.sweptAABB,
                h
            );
    ...
    // ------------------------------------
    // DSU root -> PhysicsScope
    // ------------------------------------
    std::unordered_map<int, int> rootToScope;
    rootToScope.reserve(dsu.parent.size());

    for (int i = 0; i < static_cast<int>(dsu.parent.size()); ++i) {
        int root = dsu.find(i);

        auto it = rootToScope.find(root);

        if (it == rootToScope.end()) {
            PhysicsScope scope;
            scope.type = PhysicsScopeType::Island;
            scope.id =
                static_cast<uint32_t>(islandScopes.size());
            scope.substeps = 1;

            islandScopes.push_back(std::move(scope));

            int scopeIndex =
                static_cast<int>(islandScopes.size()) - 1;

            rootToScope[root] = scopeIndex;
            it = rootToScope.find(root);
        }

        PhysicsScope& scope =
            islandScopes[it->second];

        RigidBodyHandle bodyH = dsu.bodies[i];

        scope.bodies.push_back(bodyH);

        scope.substeps = std::max(
            scope.substeps,
            dsu.wantedSubsteps[i]
        );
