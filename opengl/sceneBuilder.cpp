...
void SceneBuilder::createScene(PhysicsEngine& physicsEngine, std::vector<Vertex>& cubeVertices, std::vector<unsigned int>& indices)
{
    ...
    // projectile
    createObject(physicsEngine, "crate", glm::vec3(410, 34.5, 200), glm::vec3(5, 5, 5), 1, 0, cubeVertices, indices);
    // counterweight
...
GameObject& SceneBuilder::createObject(PhysicsEngine& physicsEngine, const std::string& textureName, glm::vec3 pos, glm::vec3 size, float mass, bool isStatic, std::vector<Vertex>& cubeVertices, std::vector<unsigned int>& indices)
{
