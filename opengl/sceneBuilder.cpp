...
void SceneBuilder::createScene(PhysicsEngine& physicsEngine, std::vector<GameObject>& GameObjectList, std::vector<Vertex>& cubeVertices, std::vector<unsigned int>& indices)
{
    ...
    // floor
    createObject(physicsEngine, GameObjectList, "uvmap", glm::vec3(250, -5, 250), glm::vec3(500, 10, 500), 0, 1, cubeVertices, indices);

    //// creating 100 tiles of floor 
    //for (int i = 0; i < 10; i++)
    //    for (int j = 0; j < 10; j++)
    //        createObject(physicsEngine, GameObjectList, "uvmap", glm::vec3(250+i*500, -5, 250+j * -500), glm::vec3(500, 10, 500), 0, 1, cubeVertices, indices);

    // player controlled box
    createObject(physicsEngine, GameObjectList, "crate", glm::vec3(245, 60, 100), glm::vec3(10, 10, 10), 1, 0, cubeVertices, indices);

    // slanted platform
    createObject(physicsEngine, GameObjectList, "crate", glm::vec3(245, 30, 100), glm::vec3(40, 2, 40), 0, 1, cubeVertices, indices);
    GameObjectList[objectId - 1].orientation = glm::angleAxis(glm::radians(25.0f), glm::vec3(1.0f, 0.5f, 0.0f));

    //---catapult---
    // support
    createObject(physicsEngine, GameObjectList, "crate", glm::vec3(345, 15, 200), glm::vec3(5, 30, 20), 1, 1, cubeVertices, indices);
    // plank
    int mass = 50;
    glm::vec3 size = glm::vec3(140, 2, 5);
    createObject(physicsEngine, GameObjectList, "crate", glm::vec3(345, 31, 200), size, mass, 0, cubeVertices, indices);
    ...
    // projectile
    createObject(physicsEngine, GameObjectList, "crate", glm::vec3(410, 34.5, 200), glm::vec3(5, 5, 5), 1, 0, cubeVertices, indices);
    // counterweight
    createObject(physicsEngine, GameObjectList, "crate", glm::vec3(282.5, 200, 200), glm::vec3(12, 12, 12), 100, 0, cubeVertices, indices);

    // box stacks
    for (int j = 0; j < amountStacks; j++)
        for (int i = 0; i < amountObjects; i++)
            createObject(physicsEngine, GameObjectList, "crate", glm::vec3(245 + j * 10.2, (10 * i) + 5, 245), glm::vec3(10, 10, 10), 1, 0, cubeVertices, indices);
...
void SceneBuilder::createObject(PhysicsEngine& physicsEngine, std::vector<GameObject>&GameObjectList, const std::string& textureName, glm::vec3 pos, glm::vec3 size, float mass, bool isStatic, std::vector<Vertex>& cubeVertices, std::vector<unsigned int>& indices)
{
